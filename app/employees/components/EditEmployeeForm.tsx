...
export default function EditEmployeeForm({
  ...
  useEffect(() => {
    ...
    if (!canEdit) {
      setError("Unauthorized: You do not have permission to edit employees.");
      return;
    }

    const fetchData = async () => {
      try {
        // Fetch employee data and supervisors in parallel
        const [employeeData, supervisorsData] = await Promise.all([
          apiCall(getEmployee(employeeId), { showLoading: true }),
          apiCall(fetchEmployees(), { showLoading: true }),
        ]);
        ...
        const validSupervisors = supervisorsData.filter((emp) => {
    ...
    fetchData();
  }, [employeeId, userRole, currentEmployeeId, isInitialized, canEdit]);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      await apiCall(updateEmployee(employeeId, formData), {
        showLoading: true,
      });
        ...
        <div className="bg-white rounded-lg border p-6 shadow-sm">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <div className="flex items-center space-x-2 mt-1">
                  ...
                  <Badge variant={getStatusVariant(formData.Status) as any}>
                    {formData.Status}
                  </Badge>
                </div>
              </div>
            ...
            <div className="text-right">
              {/* Removed Employee ID and Joined Date from header */}
            </div>
          </div>
        </div>
      </div>
